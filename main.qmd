...
En este tipo de matrices, las diagonales indican la varianza de cada variable. Por ello se obtiene que:
- Var(KB) = 14.28
- Var(EL) = 6.05
Es decir, KB varía mucho más que EL. Esto es debido a que en la muestra escogida los valores de KB tienen mayor dispersión (varían más) que los valores de EL. 

Por otro lado, el resto de variables indican la covarianza entre la variable fila y la variable columna. En este caso, al solo tener dos variables se obtiene la covarianza de KB y EL:
- Cov(KB, EL) = 8.20
Al ser un valor positivo de covarianza se puede afirmar que, cuando una de las variables crece, la otra también lo hace. Es decir, la anchura de la cabeza crece con la longitud de las elitros y viceversa. Al ser un valor relativamente algo, se ve que esta relación es destacable.
